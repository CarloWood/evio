...
class EventLoopThread : public Singleton<EventLoopThread>
{
  ...
  friend_Instance;
  EventLoopThread() : m_epoll_fd(-1), m_epoll_signum(utils::Signals::reserve_and_next_rt_signum()),
      /*m_inside_invoke_pending(false),*/ m_active(0), m_terminate(not_yet), m_running(false) { }
  ~EventLoopThread();
...
//  void handle_invoke_pending();
 ...
 private:
  ...
  void stop_if(FileDescriptor::state_t::wat const& state_w, utils::FuzzyCondition const& condition, FileDescriptorFlags::mask_t active_flag, FileDescriptor* device);
 ...
 public:
  ...
  [[gnu::always_inline]] void stop_if(FileDescriptor::state_t::wat const& state_w, utils::FuzzyCondition const& condition, InputDevice* input_device) { stop_if(state_w, condition, FileDescriptorFlags::FDS_R_ACTIVE, input_device); }
  [[gnu::always_inline]] void stop_if(FileDescriptor::state_t::wat const& state_w, utils::FuzzyCondition const& condition, OutputDevice* output_device) { stop_if(state_w, condition, FileDescriptorFlags::FDS_W_ACTIVE, output_device); }
